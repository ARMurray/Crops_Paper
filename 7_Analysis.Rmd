...
ppt$GSpptCategories <- ifelse(ppt$GrowingSeasonAnomaly >= -1.0 & ppt$GrowingSeasonAnomaly <= 1.0, "Near Normal", ifelse(ppt$GrowingSeasonAnomaly > 1.0, "Above Normal", ifelse(ppt$GrowingSeasonAnomaly < -1.0, "Below Normal", NA)))

ppt$GSTempCategories <- ifelse(ppt$GrowingSeasonTempAnomaly >= -1.0 & ppt$GrowingSeasonTempAnomaly <= 1.0, "Near Normal", ifelse(ppt$GrowingSeasonTempAnomaly > 1.0, "Above Normal", ifelse(ppt$GrowingSeasonTempAnomaly < -1.0, "Below Normal", NA)))
...
cornpptcorr <- ddply(corn, .(State), summarise,corr=corfun(GrowingSeasonpptAnomaly, YieldPercentChange)$estimate,
      pval=corfun(GrowingSeasonpptAnomaly, YieldPercentChange)$p.value) 
corntempcorr <- ddply(corn, .(State), summarise,corr=corfun(GrowingSeasontempAnomaly, YieldPercentChange)$estimate,
      pval=corfun(GrowingSeasontempAnomaly, YieldPercentChange)$p.value) 

cottonpptcorr <- ddply(cotton, .(State), summarise,corr=corfun(GrowingSeasonpptAnomaly, YieldPercentChange)$estimate,
      pval=corfun(GrowingSeasonpptAnomaly, YieldPercentChange)$p.value) 
cottontempcorr <- ddply(cotton, .(State), summarise,corr=corfun(GrowingSeasontempAnomaly, YieldPercentChange)$estimate,
      pval=corfun(GrowingSeasontempAnomaly, YieldPercentChange)$p.value) 

peanutspptcorr <- ddply(peanuts, .(State), summarise,corr=corfun(GrowingSeasonpptAnomaly, YieldPercentChange)$estimate,
      pval=corfun(GrowingSeasonpptAnomaly, YieldPercentChange)$p.value) 
peanutstempcorr <- ddply(peanuts, .(State), summarise,corr=corfun(GrowingSeasontempAnomaly, YieldPercentChange)$estimate,
      pval=corfun(GrowingSeasontempAnomaly, YieldPercentChange)$p.value) 

soybeanspptcorr <- ddply(soybeans, .(State), summarise,corr=corfun(GrowingSeasonpptAnomaly, YieldPercentChange)$estimate,
      pval=corfun(GrowingSeasonpptAnomaly, YieldPercentChange)$p.value) 
soybeanstempcorr <- ddply(soybeans, .(State), summarise,corr=corfun(GrowingSeasontempAnomaly, YieldPercentChange)$estimate,
      pval=corfun(GrowingSeasontempAnomaly, YieldPercentChange)$p.value) 

sweetPotatoespptcorr <- ddply(sweetPotatoes, .(State), summarise,corr=corfun(GrowingSeasonpptAnomaly, YieldPercentChange)$estimate,
      pval=corfun(GrowingSeasonpptAnomaly, YieldPercentChange)$p.value) 
sweetPotatoestempcorr <- ddply(sweetPotatoes, .(State), summarise,corr=corfun(GrowingSeasontempAnomaly, YieldPercentChange)$estimate,
      pval=corfun(GrowingSeasontempAnomaly, YieldPercentChange)$p.value) 
...
p <- ggviolin(soybeans, x = "octpptDays99", y = "YieldPercentChange",
               fill = "GSTempCategories", shape = "GSTempCategories", palette =c("#FC4E07","#E7B800","#00AFBB"), add = "boxplot", add.params = list(fill = "white") , order = c("Below Normal", "Near Normal", "Above Normal")) + scale_x_discrete( 
                     labels = c('Below\nNormal', 'Near\nNormal', 'Above\nNormal'))
...
p2 <- ggpar(p, 
            ...
            legend.title = "     Growing Season \n Precipitation") + theme(plot.title = element_text(hjust = 0.5), legend.position = "none")
p2


p3 <- p2 + facet_wrap(. ~ State) + stat_compare_means(method = "kruskal.test", label = "p.format", label.y = 115) + stat_compare_means(label = "p.signif", method = "wilcox.test", ref.group = "Near Normal", label.y = 100)  

soybeanspptplot <- p3


#ggsave(filename = "cornppt.tiff", plot = cornpptplot)
```
